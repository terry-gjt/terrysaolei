...
import static com.example.terry_app.common.Safe.decipher;
import static com.example.terry_app.common.Safe.getfront;
import static com.example.terry_app.common.Safe.getfrontnum;
import static com.example.terry_app.common.Safe.getrear;

public class DoubleclientActivity extends Activity implements View.OnClickListener,View.OnLongClickListener {
    // 最外层布局
    ...
    // -1
    Set<Integer> random地雷=new HashSet<Integer>();;
    List<Integer> 旗子 = new ArrayList<Integer>();
    Handler clienthander;
    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        this.requestWindowFeature(Window.FEATURE_NO_TITLE);//去掉标题栏
//        this.getWindow().setFlags(WindowManager.LayoutParams.FLAG_FULLSCREEN, WindowManager.LayoutParams.FLAG_FULLSCREEN);//去掉信息栏
        setContentView(R.layout.activity_doubleclient);
        inithander();
        miyao = (EditText) findViewById(R.id.miyao);
        miyao.setText("257133");
        mess=(TextView)findViewById(R.id.mess) ;
        join = (Button) findViewById(R.id.join);
        join.setOnClickListener(new View.OnClickListener(){
            @Override
            public void onClick(View a) {
                String s=miyao.getText().toString();
                Connection=new Connection(decipher(s));
                Connection.sethandler(clienthander);
                Connection.startconnect();
                miyao.setEnabled(false);
                join.setEnabled(false);
                mess.setText("等待对方操作...");
            }
        });
        textviews = (LinearLayout) findViewById(R.id.textviews2);
        buttons = (LinearLayout) findViewById(R.id.buttons2);
//        initData();
//        initView();
    }
    @Override
        ...
        clienthander=new Handler()
        {
            public void handleMessage(Message msg)
            {
                ...
                if(msg.what==1)
                {
                    ...
                    Log.i("线程运行doubleclient","收到消息"+MessageServer+"");
                    String ff=getfront(MessageServer);
                    Log.i("线程运行doubleclient","ff为"+ff+"");
                    switch (ff){
                        case "0101":
                            String gg;
                            random地雷.clear();
                            gg=getrear(MessageServer);
                            Log.i("线程运行doubleclient","gg去首为"+gg+"");
                            for(int j=0;j<10;j++){
                                int i=getfrontnum(gg);
                                Log.i("线程运行doubleclient","添加地雷"+i+"");
                                random地雷.add(i);
                                gg=getrear(gg);
                                Log.i("线程运行doubleclient","gg为"+gg+"");
                            }
                            initData();
                            initView();
                            break;
                        ...
                        case "0104"://restart
                            String ccc;
                            random地雷.clear();
                            ccc=getrear(MessageServer);
                            Log.i("线程运行doubleclient","ccc去首为"+ccc+"");
                            for(int j=0;j<10;j++){
                                int i=getfrontnum(ccc);
                                Log.i("线程运行doubleclient","添加地雷"+i+"");
                                random地雷.add(i);
                                ccc=getrear(ccc);
                                Log.i("线程运行doubleclient","ccc为"+ccc+"");
                            }
                            restart();
                            mess.setText("等待对方操作...");
                            break;
